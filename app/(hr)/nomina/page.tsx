...
import { NominaEstadoBadge, EstadoAgenteBadge, LicenciaBadge, CambioTemporalBadge, VacacionBadge } from '@/components/hr/ui/Badge'
import { PageLoading } from '@/components/hr/ui/LoadingSpinner'
import EditAgentModal from '@/components/hr/agents/EditAgentModal'
import LicenciaModal from '@/components/hr/agents/LicenciaModal'
import CambioTemporalModal from '@/components/hr/agents/CambioTemporalModal'
...
export default function Nomina() {
  ...
  // Local state (replacing useSearchParams + setSearchParams)
  ...
  const [selectedTipo, setSelectedTipo] = useState('OPERACION')
  ...
  const columns = useMemo<ColumnDef<AgenteNominaMensual>[]>(() => [
    ...
    },
    {
      ...
      cell: ({ row }) => (
        ...
        >
          <button
            onClick={() => router.push(`/nomina/agente/${row.original.agente_id}`)}
  ...
  // router y handleDeleteAgente son siempre estables → columns NUNCA se reconstruye
  // eslint-disable-next-line react-hooks/exhaustive-deps
  ], [router, handleDeleteAgente])
          ...
          <div className="flex items-center gap-2">
            {isAdmin && selectedServicioId && (
              <button
                ...
                onClick={() =>
                  router.push(`/carga?servicio_id=${selectedServicioId}&mes=${selectedMes}&anio=${selectedAnio}&formato=${selectedTipo === 'MEUCCI' ? 'meucci' : 'operacion'}`)
                }
              >
          ...
          <div className="flex-1 flex flex-col items-center justify-center gap-4">
            ...
            {isAdmin && (
              <button
                ...
                onClick={() => router.push(`/carga?servicio_id=${selectedServicioId}&mes=${selectedMes}&anio=${selectedAnio}`)}
              >
