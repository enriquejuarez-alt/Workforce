...
const ALL_COMMANDS: Command[] = [
  ...
  { to: '/planificacion',           label: 'Planif. ? Carga de archivos', section: 'Walt', icon: UploadCloud },
  { to: '/planificacion/resumen',   label: 'Planif. ? Resumen',           section: 'Walt', icon: BarChart3 },
  { to: '/planificacion/curvas',    label: 'Planif. ? Curvas',            section: 'Walt', icon: TrendingUp },
  { to: '/planificacion/simulador', label: 'Planif. ? Simulador',         section: 'Walt', icon: Sliders },
  { to: '/planificacion/contratos', label: 'Planif. ? Contratos',         section: 'Walt', icon: FilePen },
...
export default function CommandPalette({ isAdmin = false }: Props) {
  ...
  const handleKeyDown = (e: React.KeyboardEvent) => {
    ...
    else if (e.key === 'Enter' && results[selectedIdx]) { router.push(results[selectedIdx].to); close() }
            ...
            results.map((cmd, i) => {
              ...
              return (
                <button
                  ...
                  onClick={() => { router.push(cmd.to); close() }}
