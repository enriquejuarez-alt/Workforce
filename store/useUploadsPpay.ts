...
interface UploadsPpayState {
  ...
  setModoReductor: (m: "multiplicativo" | "aditivo") => void;
  setTopeFacturacion: (v: number) => void;
  reset: () => void;
}

export const useUploadsPpay = create<UploadsPpayState>((set) => ({
  archivoCP: null,
  ...
  setArchivoNomina: (f, hojas) =>
    set({ archivoNomina: f, hojasNomina: hojas, hojaNomina: hojas[0] ?? null }),
